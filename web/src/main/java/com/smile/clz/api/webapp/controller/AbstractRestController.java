...
public class AbstractRestController {
    ...
    public Error handleException(Exception e, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (e instanceof ClassApiException) {
            ...
            String message = this.messages.getMessage(ERRORS + ((ClassApiException) e).getHttpStatusCode().value(), new Object[]{}, e.getMessage(), locale);
            return getError(((ClassApiException) e).getHttpStatusCode().value(), ((ClassApiException) e).getErrorCode(), message, ((ClassApiException) e).getHttpStatusCode().toString(), response);
        } else {
